  ...
  return (
    <>
      <div className="card mb-4 p-4">
        <div className="grid gap-3 md:grid-cols-[minmax(220px,1fr)_220px_120px_minmax(160px,1fr)_120px_auto]">
          <UnitSelect value={form.unitPendidikanId} units={unit.data?.items ?? []} onChange={(value) => setForm({ ...form, unitPendidikanId: value })} />
          <Input label="Tahun ajaran ID" value={form.tahunAjaranId} onChange={(value) => setForm({ ...form, tahunAjaranId: value })} />
          ...
          <div className="flex items-end"><button type="button" className="btn-primary" disabled={!form.unitPendidikanId || !form.tahunAjaranId || !form.tingkat || !form.nama || simpan.isPending} onClick={() => simpan.mutate()}><Plus className="h-4 w-4" aria-hidden />Tambah</button></div>
        </div>
      </div>
...
function TabKurikulum() {
  ...
  const simpan = useMutation({
    mutationFn: () => api.post('/pesantren/kurikulum', { ...form, jamPerMinggu: Number(form.jamPerMinggu) }),
    onSuccess: () => {
  ...
  return (
    <>
      <div className="card mb-4 p-4">
        <div className="grid gap-3 md:grid-cols-[minmax(220px,1fr)_220px_120px_minmax(220px,1fr)_120px_auto]">
          <UnitSelect value={form.unitPendidikanId} units={unit.data?.items ?? []} onChange={(value) => setForm({ ...form, unitPendidikanId: value })} />
          <Input label="Tahun ajaran ID" value={form.tahunAjaranId} onChange={(value) => setForm({ ...form, tahunAjaranId: value })} />
          <Input label="Tingkat" value={form.tingkat} onChange={(value) => setForm({ ...form, tingkat: value })} />
          <div>
          ...
          <div className="flex items-end"><button type="button" className="btn-primary" disabled={!form.unitPendidikanId || !form.tahunAjaranId || !form.tingkat || !form.mataPelajaranId || simpan.isPending} onClick={() => simpan.mutate()}><Plus className="h-4 w-4" aria-hidden />Tambah</button></div>
        </div>
      </div>
      <div className="card mb-4 p-4">
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <Input label="Tahun ajaran ID" value={filter.tahunAjaranId} onChange={(value) => setFilter({ ...filter, tahunAjaranId: value })} />
