...
| 4a | Permission `DISCOUNT_LINE` | **FIXED (2026-08-08)** | `manualDiscount` pada `tambahBaris` dan `/price/quote` sekarang menuntut `POS_SALE.DISCOUNT_LINE`, mengikuti pola `PRICE_OVERRIDE` yang sudah ada persis |
| 4b | `DISCOUNT_CART` | **MISSING, lebih dalam dari dugaan semula** | Bukan sekadar permission belum dicek — fiturnya sendiri **tidak tersambung sama sekali**. `totalKeranjang()` di `pos-pricing.ts` menerima parameter `diskonKeranjang` (diskon level keranjang), tapi satu-satunya pemanggilnya di `pos-sale.service.ts:1141` memanggilnya dengan HANYA 2 argumen — parameter itu selalu default `[]`. Tidak ada DTO, endpoint, atau kolom yang mengisinya. Menambahkan permission check untuk fitur yang tidak ada tidak ada gunanya; ini butuh desain endpoint baru (bagaimana diskon keranjang berinteraksi dengan diskon per-baris yang sudah ada, pembulatan, audit) — sengaja TIDAK dikerjakan tergesa-gesa dalam gelombang ini. |
